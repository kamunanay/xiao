...
⣿⣿⣿⣿⣿⣷⡘⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡌⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
    ...
    console.clear();
    console.log(banner);
    console.log(`🚀 ${botName} aktif! Ketik ${prefix}menu di WhatsApp`.green);
}
...
async function connectToWhatsApp(sessionId = 'default') {
    try {
        ...
        newSock.ev.on('connection.update', async (update) => {
            ...
            if (qr) {
                console.log('📱 Scan QR code di atas untuk login!'.yellow);
            }

            if (connection === 'close') {
                const shouldReconnect = lastDisconnect?.error?.output?.statusCode !== 401;
                console.log('Connection closed, reconnecting...'.red, shouldReconnect);
                if (shouldReconnect) {
                    await connectToWhatsApp(sessionId);
                }
            } else if (connection === 'open') {
                console.log('✅ Berhasil terhubung ke WhatsApp!'.green);
...
async function getCecanImage(query) {
    try {
        ...
        console.log('Cecan error:'.red, error.message);
        return null;
    }
}
...
async function getBratImage(text) {
    try {
        ...
        console.log('Brat error:'.red, error.message);
        return null;
    }
}
...
async function removeBackground(imageBuffer) {
    try {
        ...
        throw new Error('RemoveBG API key required. Set environment variable RMBG_API');
    } catch (error) {
        throw new Error(`RemoveBG failed: ${error.message}`);
    }
}
...
⇴ ├─▰─ Pairing System
⇴ ├─▰─ Owner Access
⇴ │├─ ${prefix}pair <nomor> - Pairing WhatsApp
⇴ │├─ ${prefix}restart - Restart bot
⇴ │└─ ${prefix}listsession - List session
⇴ └─────────────────────────
...
📌 Query cecan: indonesia, china, thailand, vietnam, malaysia, japan, korea, waifu, neko
...
async function messageHandler(sock, m) {
    try {
        if (m.messages && m.messages[0]) {
            ...
            // Auto reaction for status
            if (jid.endsWith('@status.us')) {
            ...
            console.log(`[PESAN] dari ${jid}: ${body}`.blue);
...
🔗 Source: Private Repository
                ...
                // 📱 PAIRING COMMAND
                if (command === 'pair') {
                    if (!isValidPhoneNumber(args)) {
                        await sock.sendMessage(jid, { text: '❌ Format nomor salah!\nGunakan: 628xxxxxxx' });
                        return;
                    }
                    await sock.sendMessage(jid, { text: `📱 Memulai pairing untuk ${args}...\nScan QR code di terminal!` });
                    await connectToWhatsApp(args);
                    return;
                }
                ...
                if (command === 'listsession') {
                    const sessionList = Array.from(sessions.keys()).map(s => `• ${s}`).join('\n');
                    await sock.sendMessage(jid, { 
                        text: sessionList ? `📱 Active Sessions:\n${sessionList}` : '📭 Tidak ada session aktif' 
                    });
                    return;
                }
                ...
                if (command === 'rmbg') {
                    await sock.sendMessage(jid, { 
                        text: '❌ Fitur RemoveBG sedang maintenance.\nButuh API key dari remove.bg' 
                    });
                    return;
                }
        ...
        console.log('Handler error:'.red, error);
    }
}
...
async function initialize() {
    try {
        ...
        console.log('🚀 Starting XIS CORE SYSTEMS...'.yellow);
        
        // Connect to WhatsApp
        await connectToWhatsApp();
        
        console.log('✅ Bot berhasil diinisialisasi!'.green);
        
    } catch (error) {
        console.log('❌ Gagal memulai bot:'.red, error);
        process.exit(1);
    }
}
