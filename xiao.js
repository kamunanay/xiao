...
async function getPapImage() {
    try {
        ...
        const apiUrl = 'https://restapi-v2.simplebot.my.id/random/papayan';
...
// === WHATSAPP CONNECTION WITH PAIRING CODE 8 DIGIT ===
async function connectToWhatsApp() {
    try {
        ...
        newSock.ev.on('connection.update', async (update) => {
            const { connection, lastDisconnect, qr, pairingCode, phoneNumber } = update;
            ...
            if (pairingCode && phoneNumber) {
                ...
                console.log(`4. Masukkan nomor: ${phoneNumber}`);
                console.log(`5. Masukkan kode 8 digit: ${pairingCode}`);
                console.log('='.repeat(60) + '\n');
                reconnectAttempts = 0;
            }
            ...
            if (qr) {
                console.log('\n' + '='.repeat(60));
                console.log('📱 QR CODE (Backup System)');
                console.log('='.repeat(60));
                console.log('Jika pairing code tidak muncul, scan QR ini:');
                console.log('1. Buka WhatsApp → Linked Devices → Link a Device');
                console.log('2. Scan QR code di terminal lain');
                console.log('='.repeat(60));
                console.log('🔄 Menunggu kode pairing 8 digit...');
                console.log('='.repeat(60) + '\n');
                reconnectAttempts = 0;
            }
        ...
        newSock.ev.on('connection.update', (update) => {
            if (update.error) {
                console.log('⚠️ Connection error:', update.error);
                // Auto reset jika error critical
                if (update.error.message.includes('Stream Errored') ||
                    update.error.message.includes('ECONNREFUSED') ||
                    update.error.message.includes('timed out')) {
                    console.log('🔄 Auto reset karena error critical...');
                    setTimeout(() => resetSender(), 5000);
                }
            }
        });
...
function askPhoneNumber() {
    return new Promise((resolve) => {
        console.log('\n' + '='.repeat(60));
        console.log('📱 MASUKKAN NOMOR TELEPON');
        console.log('='.repeat(60));
        console.log('Format: 628xxxxxxxxxx (tanpa +)');
        console.log('Contoh: 6281234567890');
        console.log('Note: Pairing code 8 digit akan muncul setelah ini');
        console.log('='.repeat(60));
        
        rl.question('Nomor Telepon: ', (phoneNumber) => {
            if (!phoneNumber.startsWith('628')) {
                console.log('❌ Format nomor salah! Harus dimulai dengan 628');
                console.log('Contoh: 6281234567890');
                process.exit(1);
            }
            
            if (phoneNumber.length < 10 || phoneNumber.length > 15) {
                console.log('❌ Panjang nomor tidak valid!');
                process.exit(1);
            }
            
            console.log(`✅ Nomor diterima: ${phoneNumber}`);
            console.log('🔄 Memulai proses pairing 8 digit...\n');
            resolve(phoneNumber);
        });
    });
}
