...
async function deepseekAI(text) {
    try {
        ...
        const data = response.data;
        console.log('📦 Data lengkap:', JSON.stringify(data).substring(0, 200) + '...');
        ...
        if (typeof data === 'string') {
            // Jika respons berupa string langsung
            aiResponse = data;
        } else if (data.result) {
            // Jika ada property result
            aiResponse = typeof data.result === 'string' ? data.result : JSON.stringify(data.result);
        } else if (data.message) {
            // Jika ada property message
            aiResponse = data.message;
        } else if (data.text) {
            // Jika ada property text
            aiResponse = data.text;
        } else if (data.data) {
            // Jika ada property data
            aiResponse = typeof data.data === 'string' ? data.data : JSON.stringify(data.data);
        } else {
            // Jika tidak ada format yang dikenali, stringify seluruh data
            aiResponse = JSON.stringify(data);
        }
        
        // Bersihkan respons jika terlalu panjang
            ...
            switch(command) {
                ...
                case 'nomor':
                    ...
                    try {
                        ...
                        if (data.kartu) resultText += `💳 *Kartu:* ${data.kartu}\n`;
                        if (data.status) resultText += `✅ *Status:* ${data.status}\n`;
                        if (data.negara) resultText += `🌍 *Negara:* ${data.negara}\n`;
                        if (data.kode_negara) resultText += `🇮🇩 *Kode Negara:* ${data.kode_negara}\n`;
