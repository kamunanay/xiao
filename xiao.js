...
// === PAIRING SYSTEM DENGAN PAIRING CODE ===
    ...
    try {
        ...
        newSock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                try {
                    await newSock.sendMessage(ownerJid, { 
                        text: `🤖 *${botName} Active!*\n\n📱 Number: ${phoneNumber}\n✅ Status: Connected\n\nKetik ${prefix}menu untuk melihat daftar perintah.` 
                    });
                } catch (error) {
                    console.log('Tidak bisa mengirim pesan ke owner');
                }
            } else if (connection === 'connecting') {
                console.log('🔄 Connecting to WhatsApp...');
                
                // Request pairing code if no credentials exist
                if (!fs.existsSync(path.join(sessionPath, 'creds.json'))) {
                    try {
                        pairingCode = await newSock.requestPairingCode(phoneNumber);
                        console.log(`\n📱 *PAIRING CODE UNTUK ${phoneNumber}*`);
