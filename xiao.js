...
// === PAIRING SYSTEM YANG BENAR ===
async function connectToWhatsApp(phoneNumber) {
    return new Promise(async (resolve, reject) => {
        try {
            const sessionPath = path.join(sessionsDir, `device_${phoneNumber}`);
            ...
            newSock.ev.on('connection.update', async (update) => {
                ...
                if (qr) {
                    ...
                    console.log('Atau gunakan pairing code di bawah ini:\n');
                }
                ...
                if (connection === 'close') {
                    ...
                    if (statusCode === 401) {
                        ...
                        await delay(2000);
                        connectToWhatsApp(phoneNumber).then(resolve).catch(reject);
                    } else {
                        await delay(3000);
                        connectToWhatsApp(phoneNumber).then(resolve).catch(reject);
                    }
                    ...
                    try {
                        ...
                        console.log('Tidak bisa kirim pesan ke owner');
                    }
                }
                else if (connection === 'connecting') {
                    console.log('🔄 Connecting to WhatsApp...');
                    
                    // Request pairing code jika belum ada kredensial
                    if (!pairingCodeRequested && Object.keys(state.creds).length === 0) {
                        try {
                            pairingCodeRequested = true;
                            const code = await newSock.requestPairingCode(phoneNumber);
                            console.log('\n══════════════════════════════════════');
                            console.log('📱 *PAIRING CODE UNTUK ' + phoneNumber + '*');
                            console.log('🔢 KODE: ' + code);
                            console.log('══════════════════════════════════════\n');
                            console.log('💡 Cara pakai:');
                            console.log('1. Buka WhatsApp di HP');
                            console.log('2. Pergi ke Settings → Linked Devices → Link a Device');
                            console.log('3. Masukkan kode di atas');
                            console.log('══════════════════════════════════════\n');
                        } catch (pairError) {
                            console.log('❌ Gagal minta pairing code:', pairError.message);
                        }
                    }
                }
            });
...
// === INITIALIZE BOT ===
async function initialize(phoneNumber) {
    try {
        showBanner();
        ...
        // Connect to WhatsApp with phone number
